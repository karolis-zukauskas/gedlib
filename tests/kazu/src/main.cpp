...
void test_on_dataset(const std::string & dataset) {
  ...
  std::vector<ged::Options::GEDMethod> ged_methods {
    ...
    // ged::Options::GEDMethod::RING, // Really slow + training ..
  ...
  result_filename += dataset + "_results_2.csv";
  ...
  // }
  result_file << "lb, ub, runtime, num_nodes, method" << std::endl;
  result_file.close();

  // Run the tests.
  //for (std::size_t max_size_dev_10{1}; max_size_dev_10 <= 3; max_size_dev_10++) {
  {
    // Run tests on graph of size 1..10
    std::size_t max_size_dev_10 = 1;
    ...
    for (auto & method : methods) {
      ...
      result_file << avg_lb << "," << avg_ub << "," << avg_runtime << "," << env.get_avg_num_nodes() << "," << method.name() << "\n";
      result_file.close();
    }
