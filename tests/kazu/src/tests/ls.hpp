...
void test_ls_power_graphs(std::vector<Method> const& methods, size_t num_graphs, size_t node_variance,
  ...
  std::string stats_filename = ::util::create_stats_file("ls_power");
  ...
  for (size_t graph_size : graph_sizes) {
    for (size_t edges : edges_per_node) {
      ...
      try {
        ...
        run_on_generated_dataset(methods, generate_graphs, dataset, static_cast<double>(edges), false, TEST_ONLY_UNIQUE_PAIRS, results_filename, stats_filename);
      } catch (const std::exception & error) {
